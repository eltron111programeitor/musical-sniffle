...
var tipo_flecha: String = "down"
var color_original: Color = Color.WHITE

func _ready():
	print("Down arrow ready!")
...
func buscar_text_manager():
	...
	if text_manager:
		print("TextManager encontrado para DOWN")
	else:
		print("ADVERTENCIA: TextManager no encontrado para DOWN")
...
func verificar_notas_cercanas():
	...
	if notas.size() == 0:
		...
		print("DOWN: MISS (no había notas)")
		return
	...
	for nota in notas:
		if nota.is_in_group("nota_enemiga"):
			...
			print("DOWN - Distancia: " + str(int(distancia)) + " -> " + timing_resultado)
	...
	if not nota_golpeada:
		...
		print("DOWN: " + mejor_timing + " (nota incorrecta o timing malo)")
