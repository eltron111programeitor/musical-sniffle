...
var tipo_flecha: String = "up"
var color_original: Color = Color.WHITE

func _ready():
	print("Up arrow ready!")
...
func buscar_text_manager():
	...
	if text_manager:
		print("TextManager encontrado para UP")
	else:
		print("ADVERTENCIA: TextManager no encontrado para UP")
...
func verificar_notas_cercanas():
	...
	for nota in notas:
		if nota.is_in_group("nota_enemiga"):
			...
			print("UP - Distancia: " + str(int(distancia)) + " -> " + timing_resultado)
	...
	if not nota_golpeada:
		...
		print("UP: " + mejor_timing + " (nota incorrecta o timing malo)")
